...
var worker1Url = new URL("input-worker1.js?esm", import.meta.url);
var worker2Url = new URL("input-worker2.js?esm", import.meta.url);
var count = 0;
function onMessage(e) {
	count++;
	if (count === 2) console.log("[WORKER_OK]", "both");
...
```js
import { t as sharedUtil } from "./shared.js";
//#region input/worker1.js
self.postMessage(sharedUtil("Worker 1"));
//#endregion
```
...
```js
//#region input/shared.js
function sharedUtil(name) {
	return `Hello from ${name} using shared module!`;
}
//#endregion
export { sharedUtil as t };
```
