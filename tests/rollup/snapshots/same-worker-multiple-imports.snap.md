# same-worker-multiple-imports (rollup)
...
function createWorker1() {
  const workerUrl = new URL('tests-rollup-tmp-same-worker-multiple-imports-input-worker.js?esm', import.meta.url);
  return new Worker(workerUrl, { type: 'module' });
}
...
### tests-rollup-tmp-same-worker-multiple-imports-input-worker.js
